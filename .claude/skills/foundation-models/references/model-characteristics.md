...
調査日: 2026-02-08 / 詳細データ: `references/research/01-07*.md`
...
- **日本語プロンプトでは `permissive` 推奨** (default が過度にブロック)
- `permissive` でも回避できないブロックあり (完全無効化は不可能)
...
- file-read エラー時に CLI がクラッシュする (ツールエラーをモデルに返していない)
...
| 制限 | 詳細 | 回避策 |
|------|------|--------|
| 日本語ガードレール過敏 | 文化的トピックがブロックされる | `--guardrails permissive` |
| 事実の正確性 | 特に日本語で誤りが頻出 | 事実確認が必要な用途には依存しない |
| 複雑なスキーマ | 配列内オブジェクト5+プロパティで崩壊 | スキーマを浅くフラットに |
| Shell コマンド自動生成 | 不安定 | コマンドをプロンプトに直接含める |
| file-read エラー | CLI クラッシュ | CLI 側の修正が必要 |
